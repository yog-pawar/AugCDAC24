...
const VendorDisabledSubPlanList = () => {
    const [subPlanList, setSubPlanList] = useState([]);
    const [refreshFlag, setRefreshFlag] = useState(false);
    const navigate = useNavigate();
    useEffect(() => {
        let ven = JSON.parse(sessionStorage.getItem("vendor"));
        axios.get(`${IP_ADDRS}/vendors/getAllDisabledPlanByVendorId/${ven.id}`, { headers: { "Authorization": `Bearer ${ven.jwt}` } })
            .then(res => {
    ...
    return (
        <>
            <div className="container my-4">
                <div>
                    <h3>All Disabled Subscription Plans</h3>
...
export default VendorDisabledSubPlanList;